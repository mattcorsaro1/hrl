...
def plot_initiation_distribution(option, mdp, episode, experiment_name, chunk_size=10000):
    ...
    saving_path = os.path.join(RESULT_DIR, 'results', experiment_name, 'initiation_set_plots', f'{option.name}_initiation_distribution_{episode}.png')
    plt.savefig(saving_path)
    plt.close()
...
def make_chunked_goal_conditioned_value_function_plot(solver, goal, episode, seed, experiment_name, chunk_size=1000, replay_buffer=None, option_idx=None):
    ...
    saving_path = os.path.join(RESULT_DIR, 'results', experiment_name, 'value_function_plots', f'{file_name}.png')
...
def plot_value_distribution(solver, state, goal, episode, option_name, seed, experiment_name):
    ...
    saving_path = os.path.join(RESULT_DIR, 'results', experiment_name, 'value_function_plots', f'{fname}')
